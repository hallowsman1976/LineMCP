...
- พิมพ์ `<เลขที่สมาชิก> <เลขบัตรประชาชน>` เช่น `<เลขที่สมาชิก> <เลขบัตร 13 หลัก>` (ใช้ข้อมูลจริงจาก Members ที่ import แล้ว — ห้ามเขียนเลขจริงลงในเอกสาร)
  ...
  (ทาง LIFF จะเจอหน้ายินยอม PDPA ก่อนเสมอ — กดยินยอมแล้วจะบันทึก `PDPAConsentAt` ลงชีต Members ให้อัตโนมัติ
  **ต้องรัน `ensureSchema()` หรือ `initializeProject()` ซ้ำ 1 ครั้งหลัง deploy รอบนี้ เพื่อเพิ่มคอลัมน์ `PDPAConsentAt` ในชีต Members ที่มีอยู่แล้ว**
  และแก้ข้อความ `[กรุณาระบุช่องทางติดต่อเจ้าหน้าที่คุ้มครองข้อมูลส่วนบุคคล...]` ใน `frontend/liff.html` ให้เป็นช่องทางติดต่อจริงก่อนใช้งานจริง)
